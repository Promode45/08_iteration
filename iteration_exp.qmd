...
```{r}
meancv <- function(data, var){
  
  # Calculate intermediary steps of mean and sd
  mean <-summarize(mean({{var}}, na.rm = T))
  sd <- summmarize(sd({{var}}, na.rm = T))
  # Calculate cv
 # cv <- (sd/mean)*100
...
nematode_meancv$cv[[i-2]] <- statsum$cv
  # Print a message
print(paste0("Finished with variable ", varname))
}
```
double brackets in nematode_w[[i]], is to extract the values under the column. If its just one bracket it gives back the whole column. 
